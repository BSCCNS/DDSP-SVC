    ...
    for epoch in range(start_epoch, args.train.epochs):
        for batch_idx, data in enumerate(loader_train):
            ...
            if dtype == torch.float32:
                ddsp_loss, diff_loss = model(data['units'].float(), data['f0'], data['volume'], conditioner=data['physical'], spk_id=data['spk_id'], 
                                aug_shift=data['aug_shift'], vocoder=vocoder, gt_spec=data['mel'].float(), 
                                infer=False, k_step=args.model.k_step_max)
            else:
                with autocast(device_type=args.device, dtype=dtype):
                    ddsp_loss, diff_loss=model(data['units'], data['f0'], data['volume'], spk_id=data['spk_id'], conditioner=data['physical'], 
                                    aug_shift=data['aug_shift'], vocoder=vocoder, gt_spec=data['mel'].float(), 
                                    infer=False, k_step=args.model.k_step_max)
